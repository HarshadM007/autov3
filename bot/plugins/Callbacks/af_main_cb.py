...
async def cb_navg(bot, update: CallbackQuery):
    ...
    if ((index_val + 1 )== max_pages) or ((index_val + 1) == len(results)): # Max Pages
        ...
        temp_results.append([
            InlineKeyboardButton("⚡ Back", callback_data=f"navigate({index_val}|back|{query})"),
            InlineKeyboardButton("🎖️Go To Next page🎖️", callback_data=f"navigate({index_val}|next|{query})")
        ])
    ...
    text=f"<i>🔗Check links At @CC_Links... Found</i> <code>{leng}</code> <i>📌Results For Your Query:</i> <code>{query}</code>"
